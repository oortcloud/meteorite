...
Project.prototype.migrate = function(fn) {
  ...
  // work from smart.lock, grabbing all versions of dependencies
  self.initFromLock();
  var complete = 0;
  _.each(self.dependencies.packages, function(pkg) {
    var name = pkg.source.config.name;
    
    if (! pkg.source.config.tag) {
      console.log("Sorry! We can't migrate packages that haven't been added from Atmosphere");
      console.log("Please remove them from your smart.json, rerun mrt install, and try again");
      process.exit(1);
    }
    
    var versionName = pkg.source.config.tag.replace(/^v/, '');
    ...
    Atmosphere.package(name, function(pkg) {
      ...
      if (! version) {
        console.error("Error: couldn't find version", versionName, 
          "of package", name, "on Atmosphere");
        process.exit(1);
      }
      
      if (! version.troposphereIdentifier) {
        console.error("Error: No troposphere version of", versionName, 
          "of package", name, "yet registered");
        process.exit(1)
      }
      
      // we are good, replace references to name with troposphereId
      packages = packages.replace(new RegExp('^' + name + '$', 'm'), version.troposphereIdentifier);

      complete += 1;
      console.log((complete + ". Updated " + name + " to " + version.troposphereIdentifier).green);
      if (complete === _.keys(self.dependencies.packages).length)
        done()
    });
  });
  ...
  var done = function() {
    ...
    _.each(self.dependencies.packages, function(pkg) {
      fs.unlinkSync(path.join(self.root, 'packages', pkg.name));
    });

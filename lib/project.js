...
Project.prototype.checkSmartJson = function(force) {
  ...
  var config = new Config(this.root), different = false;
  ...
  if (!self.dependencies.isEmpty()) {
    ...
    self.fetch(function() {
      ...
      self.meteor.installInto(self, function() {
        ...
        self.dependencies.installInto(self, fn);
      });
    });
  } else {
...
Project.prototype.execute = function(args, fn) {
  ...
  self.install(function() {
    if (self.dependencies.isEmpty() && path.exists(path.join(self.installRoot, 'meteor'))) {

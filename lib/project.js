...
Project.prototype.install = function(fn, force) {
  ...
  self.fetch(function() {
    ...
    if (fs.existsSync(self.smartJsonPath) && 
          (self.lockChanged || !fs.existsSync(self.smartLockPath)))
      self.writeLockFile();
    
    if (!self.needsToInstall())
      return fn();
    
    console.log('Installing custom custom meteor packages...');
    ...
    self.dependencies.installInto(self, function(packagesInstalled) {
      ...
      packagesInstalled && console.log('Packages installed');
      fn();
    });
  }, force);
};

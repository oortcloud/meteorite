...
Project.prototype.install = function(fn, force) {
  ...
  self.fetch(function() {
    ...
    if (!self.dependencies.isEmpty() && (self.lockChanged || !fs.existsSync(self.smartLockPath)))
      self.writeLockFile();
...
Project.prototype._isMeteorPresent = function() {
  var meteorPath = path.join(this.installRoot, 'meteor');
  return fs.existsSync(meteorPath);
};

Project.prototype._isMeteorCorrectVersion = function(fn) {

  var self = this;
  var child = exec('git rev-parse HEAD', { cwd: self.installRoot }, function (err, stdout) {
    var commit = stdout.trim();
    console.log(commit, self.meteor.source.commit, commit === self.meteor.source.commit);
    fn(commit === self.meteor.source.commit);
  });
};

Project.prototype.isMeteorInstalled = function(fn) {
  if (!this._isMeteorPresent())
    return fn(false);

  this._isMeteorCorrectVersion(fn);
};

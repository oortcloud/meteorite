...
Project.prototype.fetch = function(fn, forceUpdate) {
  ...
  self.meteor.prepare(function() {
    ...
    if (!self.dependencies.resolved()) {
      ...
      // Mike -- change this
      var forceDeps = false
      self.dependencies.resolve(forceDeps, function(err, problems) {
        
        // Mike -- probably change this too
        _.each(problems, function(problem, name) {
          console.log('Problem installing ' + name + ': ' + problem);
        });
        
        // FIXME -- we should use the same callback style everywhere
        if (err)
          throw err;
...
Project.prototype.install = function(fn, forceUpdate) {
  ...
  self.fetch(function() {
    // FIXME -- write smart lock here --> however this won't matter with PACKAGE_DIRS
    ...
    self.meteor.installInto(self, function(meteorInstalled) {
      
      // Add each package to newly installed meteor
      self.dependencies.installInto(self, function(packagesInstalled) {
        
        (meteorInstalled || packagesInstalled) && console.log('Meteor installed');

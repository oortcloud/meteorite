    ...
    if (!self.dependencies.resolved()) {
      ...
      self.dependencies.resolve(self.meteorArgs.force, function(err, conflicts) {
        ...
        _.each(conflicts, function(conflict, name) {
          console.log('Problem installing ' + name);
          console.log('    x ' + conflict);
        });
...
Project.prototype.uninstall = function() {
  ...
  console.log('Uninstalling custom meteor packages');
  fs.ensureDeletedSync(this.installRoot);
};
...
Project.prototype.install = function(fn, forceUpdate) {
  ...
  self.fetch(function() {
    console.log("Installing smart packages");
    ...
    self.dependencies.installInto(self, function(packagesInstalled) {
      console.log('Done installing smart packages');
...
Project.prototype.execute = function(args, fn) {
  ...
  console.blank();
  console.header();
  console.log('Welcome to Meteorite `~--.!~._,.');
  console.log('...¡ kick back while we make your life easier !');  
  console.divider();
  ...
  self.install(function() {
    ...
    console.divider();
    console.log("Ok, everything's ready. Here comes Meteor!");
    console.footer();
    console.blank();
...
Project.prototype._optimizeFS = function() {
  ...
  if (fs.existsSync(this.installRoot)) {
    ...
    var files = fs.readdirSync(this.installRoot);
    if (files.length > 1) {
      console.log("Yay! We're optimizing your installation!");
      _.each(files, function(file) {
        if (file !== 'packages') {
          var filePath = path.join(self.installRoot, file);

...
Project.prototype._packageJsPath = function() {
  return Package.prototype.libPath(process.cwd());
};

Project.prototype.execute = function(args, fn) {
  
  if (fs.existsSync(this._packageJsPath()))
    process.chdir(this._packageJsPath());
...
Project.prototype._optimizeFS = function() {
  ...
  if (fs.existsSync(this.installRoot)) {
    ...
    if (fs.existsSync(this.packagesRoot)) {
      var files = fs.readdirSync(this.packagesRoot);

      _.each(files, function(file) {
        var filePath = path.join(self.packagesRoot, file);

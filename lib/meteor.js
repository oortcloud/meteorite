...
Meteor = function(config) {
  ...
  this.source = Source.prepare(root, 'meteor', config);
...
Meteor.prototype.execute = function(args, packages, fn) {
  var self = this;
  
  self.ensurePrepared(function() {
    Command.execute(path.join(self.source.packagePath(), 'meteor'), args, packages, fn);
  });
};

...
// TODO #run and #create aren't DRY, #prepare has some similar logic
Meteor.prototype.run = function(fn) {
  var meteorArgs = process.argv.slice(2);
  var meteor = spawn('./.meteor/meteorite/meteor', meteorArgs);
...
Meteor.prototype.create = function(fn) {
  var meteorArgs = process.argv.slice(2);
  console.log(path.join(this.repo.path, 'meteor'));
  var meteor = spawn(path.join(this.repo.path, 'meteor'), meteorArgs);

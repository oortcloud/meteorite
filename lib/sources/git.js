...
GitSource.prototype.conflictsWith = function(otherSource) {
  if (otherSource instanceof GitSource) {
    ...
    if (otherSource.url === this.url &&
        otherSource.checkoutType === this.checkoutType &&
        otherSource.head === this.head)
      return false;
    else
      return {problem: 'has a different git source', override: false};
      
  } else if (otherSource instanceof LocalSource) {
    return {problem: 'is specified via git', override: false};
    
  } else {
    return {problem: 'is specified via git', override: true};
  }
}

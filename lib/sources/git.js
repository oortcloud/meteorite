...
GitSource.prototype._checkout = function(fn) {
  ...
  exec('git checkout ' + this.head, {cwd: this.sourcePath}, function(err, stdout, stderr) {
    if (err)
      throw "There was a problem checking out " + self.checkoutType + ": " + self.head;
    fn();
  });
};
...
GitSource.prototype.conflictsWith = function(otherSource) {
  if (otherSource instanceof GitSource) {
    ...
    if (this.equals(otherSource))
      ...
      var currentRepoHead = this.config.branch || this.config.tag || this.config.ref || 'master';
      var otherRepoHead = otherSource.config.branch || otherSource.config.tag || otherSource.config.ref || 'master';
      var currentRepo = this.config.git + '#' + currentRepoHead;
      var otherRepo = otherSource.config.git + '#' + otherRepoHead;
      var message = 'git repo (' + currentRepo + ') conflicts with git repo (' + otherRepo + ')';
    ...
    var message = 'git repo (' + this.config.git + ') conflicts with path (' + otherSource.path + ')';
    return {
      problem: message,
      override: false
    };

...
GitSource = function(basePath, name, config) {
...
GitSource.prototype.packagePath = function() {
  return path.join(this.basePath, this.packageNamespace(), this.commit, this.name);
};
...
GitSource.prototype._clone = function(fn) {
  ...
  exec('git clone ' + self.url + ' ' + this.sourcePath, function(err, stdout, stderr) {
    if (err && /refs not found/.test(err.message))
...
GitSource.prototype._checkout = function(fn) {
  spawn('git', ['checkout', this.head], {cwd: this.sourcePath}).on('exit', fn);
};

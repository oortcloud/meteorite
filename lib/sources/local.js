...
LocalSource.prototype.conflictsWith = function(otherSource) {
  if (otherSource instanceof LocalSource) {
    ...
    if (otherSource.path === this.path) 
      return false;

    else {
      var message = 'specified at two different paths (' + this.path + ', ' + otherSource.path + ')';
    ...
    return {problem: 'is specified via a path', override: true};
  }
}

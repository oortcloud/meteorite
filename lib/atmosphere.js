...
Atmosphere = {
  ...
  migratePackage: function(fn) {
    ...
    _.each(config.packages, function(pkg, i) {
      console.log(("4." + i + " Adding " + pkg.name + ' ' + pkg.version).green);
      // Lookup package on atmosphere + find troposhere name
      
      // find all references to "pkg.name" and replace with trop_name@version
    });
    
    fs.writeFileSync(path.join(newPackagePath, 'package.js'), packageJS);
    
    
    console.log("5. Publishing package".green);
    // XXX: `meteor0 publish --create`
    // XXX: decorate package on atmosphere server
    
    var homepage = config.homepage;
    console.log(("6. Setting homepage to " + homepage).green);
    // XXX: `meteor0 admin set-homepage #{homepage}`

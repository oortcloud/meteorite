...
Atmosphere = {
  ...
  migratePackage: function(fn) {
    ...
    if (meteoriteArgs._.length !== 3)
      throw "Usage: mrt migrate-package <path> <username>";
    
    var packagePath = meteoriteArgs._[1];
    var config = new Config(path.resolve(packagePath));
    
    var newPackagePath = meteoriteArgs._[2] + ':' + config.name;
    ...
    var release = 'METEOR-CORE@0.9.0-atm-preview';
    ...
    console.log(("4.0 Updating references to " + config.name + ' to ' + newPackagePath).yellow);
    packageJS = packageJS.replace(new RegExp("['\"]" + config.name + "['\"]"), 
      '"' + newPackagePath + '"');
    ...
    _.each(config.packages, function(dep, name) {
      ...
      if (dep.git || dep.path) {
        // XXX: can we?
        console.log("Error: Can't migrate packages with non atmosphere dependencies");
        process.exit(1);
      }
      ...
      Atmosphere.package(name, function(pkg) {
        ...
        if (! version) {
          console.log("Error: couldn't find version", (dep.version || pkg.latest), 
            "of package", name, "on Atmosphere");
          process.exit(1);
        }
        
        if (! version.troposphereIdentifier) {
          console.log("Error: No troposphere version of", (dep.version || pkg.latest), 
            "of package", name, "yet registered");
          process.exit(1)
        }
        
        // we are good, replace references to name with troposphere id
        packageJS = packageJS.replace(new RegExp("['\"]" + name + "['\"]"), 
          '"' + version.troposphereIdentifier + '"');
    ...
    if (_.keys(config.packages).length === 0)
      done();
  },

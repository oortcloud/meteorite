...
function PackageConflictError(message, conflicts) {
  this.name = "PackageConflictError";
  this.conflicts = conflicts;
  this.message = (message || "");
}
PackageConflictError.prototype = Error.prototype;
...
Resolver.prototype.resolvePackage = function(current, unresolved, resolved, fn) {
  ...
  if (existing) {
    ...
    if (current.conflictsWith(existing)) {
      ...
      self.conflicts[current.name] = current.toString() + " conflicts with " + existing.toString();
      ...
      if (self.forceDeps && current.overrides(existing)) {
        ...
        return fn(new PackageConflictError("Can't resolve dependencies! Use --force if you want meteorite to take a wild guess and install run your app anway.", self.conflicts));
      }
    }

...
Dependencies.prototype.installInto = function(project, fn) {
  ...
  _.each(this.packages, function(pkg) {
    installed = pkg.installInto(project) || installed;
  });
...
Dependencies.prototype.resolve = function(fn) {
  ...
  self._unresolved = _.extend({}, this.basePackages);
  ...
  var resolveStep = function() {
    // base case
    if (_.isEmpty(self._unresolved))
      return fn();
    
    var resolvingPkg = _.first(_.values(self._unresolved));
    var name = resolvingPkg.name;
    delete self._unresolved[name];
    ...
    self.resolvePackage(resolvingPkg, function(err) {
      err ? fn(err) : resolveStep();
    }); 
  }
...
Dependencies.prototype.resolvePackage = function(resolvingPkg, fn) {
  var self = this;
  
  self.packages[resolvingPkg.name] = resolvingPkg;
  
  // we need to read and resolve all dependencies, then it's simply:
  resolvingPkg.readDependencies(function() {
    resolvingPkg.dependencies.resolve(function(err) {
      if (err)
        return fn(err);
      ...
      _.each(resolvingPkg.dependencies.packages, function(newPkg, newPkgName) {
        ...
        if (newPkgName in self._unresolved)
          matched = self._unresolved[newPkgName];
            
        if (matched) {
          // we don't have to do anything, but we better make sure it's compatible
          // FIXME -- what if newPkg is more specific than matched? We need to deal with this.
          if (!matched.equals(newPkg)) {
              
            fn(err = "Can't resolve dependencies! Two versions of " + newPkgName + " depended on!");
          }
          
        } else {
          self._unresolved[newPkgName] = newPkg;
        }
      });

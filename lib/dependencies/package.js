...
Package.prototype.readAtmosphereDefinition = function(fn) {
  ...
  Atmosphere.package(self.name, function(pkg_definition) {
    ...
    self.source = new GitSource(self.basePath, {
...
Package.prototype.readDependenciesFromSource = function(fn) {
  ...
  self.source.fetch(function() {
    ...
    self.dependencies = new Dependencies(path, self.config.packages);
    fn();
  });
};
...
Package.prototype.installInto = function(project) {
  ...
  self.source.fetch(function() {
    ...
    if (fs.existsSync(packagePath) && fs.readlinkSync(packagePath) != this.libPath())
...
//   if (otherSource.config.name !== this.config.name)
//     return false;
//   
...
Package.prototype.conflictsWith = function(otherPkg) {
  ...
  // console.log(otherPkg.source);
  
  if (this._fromAtmosphere) {
    if (otherPkg._fromAtmosphere) {
      if (this.version && otherPkg.version && this.version !== otherPkg.version)
        return {problem: 'incompatible versions', override: false}
      else
        return false;
        
    } else {
      return {problem: 'one version specified via atmosphere, the other not', override: false}
    }
  }
  
  if (otherPkg._fromAtmosphere)
    return {problem: otherPkg.conflictsWith(this).problem, override: true};

...
Package.prototype.readAtmosphereDefinition = function(fn) {
  ...
  Atmosphere.package(self.name, function(pkg_definition) {
    ...
    self.version = self.version || pkg_definition.latest;
    
    var version_definition = _.find(pkg_definition.versions, function(v) { 
      return v.version === self.version; 
    });
    
    if (! /\d/.test(self.version[0]))
      throw('Version should begin with a number: ' + self.version);

    if (!version_definition)
      throw('No version ' + self.version + ' of package ' + self.name + ' was found in the atmosphere database');
...
Package.prototype.installInto = function(project, fn) {
  ...
  self.fetch(function() {
    ...
    if (! linkedTo) {
      ...
      if (self._fromAtmosphere) {
        Atmosphere.countInstall(self, function() {
          fn(true);
        });
      } else {
        fn(true);
...
// Add package name to packages/.gitignore
...
// just remove the package's symlink
//
// NOTE: that this doesn't attempt to remove the dependencies of this
// package's symlinks as it's too difficult to figure out exactly which
// should stay and which should go. Kick that can down the road..
Package.prototype.removeFrom = function(project, fn) {
  var self = this;
  
  var packagePath = path.join(project.packagesRoot, self.name);
  
  if(fs.existsSync(packagePath) && !fs.lstatSync(packagePath).isSymbolicLink()) {
    console.log(('  Package ' + self.name.bold +" can't be  " + "removed".bold + "from packages/ because it has been manually installed.\n").yellow);
  } else {
    fs.unlinkSync(packagePath);
  }
